"""Stacking classifier and regressor.
    ...
    Changes to Stacking Classifier:
        * the stacking classifier is adapted to also accept fitted base models.
        * the stacking classifier can set the maximal number of evaluations for the final estimator (if supported)
            based on a passed number of max_fun_per_base_model and the number of base models.

"""

# Authors: See original authors in sklearn code base + us
...
class _BaseStacking(TransformerMixin, _BaseHeterogeneousEnsemble, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.max_fun_per_base_model = max_fun_per_base_model
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # -- Check all base estimators are fitted
        if self.prefitted:

            # - "Stupid" and specific is fitted check for classification for now...
            for bm in all_estimators:
                check_is_fitted(bm, ["classes_"])

            self.estimators_ = [est for est in all_estimators if est != "drop"]
        else:
...
class StackingClassifier(ClassifierMixin, _BaseStacking):
    ...
    max_fun_per_base_model: int, default=-1
        If -1, we do nothing at the final estimator model is allowed to controll the number of function
        evalatuins. Otherwise, we set the number of max_fun of the final estimator to
        max_fun_per_base_model*n_base_models.
    ...
    def __init__(
        ...
        if max_fun_per_base_model > 0:
            max_fun = max_fun_per_base_model * len(estimators)
            final_estimator.set_params(**{"max_fun": max_fun})
        ...
        super().__init__(
            ...
            max_fun_per_base_model=max_fun_per_base_model
        )
        if self.prefitted:
    ...
    def predict_proba(self, X):
        ...
        # Fix for edge case where base models have seen more classes than y_true had at fit time.
        n_classes = len(self.classes_)
        if n_classes != y_pred.shape[1]:
            # Find missing classes and their indices
            assumed_f_est_classes = self._le.transform(self.classes_)
            missing_classes = set(assumed_f_est_classes) - set(self.final_estimator_.classes_)
            ind_of_missing_classes = {c: idx for idx, c in enumerate(assumed_f_est_classes) if c in missing_classes}

            # Fill a new y_pred with results from predict_proba + 0 vectors for missing classes
            tmp_y_pred = np.zeros((y_pred.shape[0], n_classes))
            class_to_index_mapping = {c: idx for idx, c in enumerate(self.final_estimator_.classes_)}
            index_map = {**class_to_index_mapping, **ind_of_missing_classes}

            for new_idx, c in enumerate(assumed_f_est_classes):

                if c in missing_classes:
                    tmp_y_pred[:, new_idx] = 0
                else:
                    tmp_y_pred[:, new_idx] = y_pred[:, index_map[c]]
